...
case(1)
E0 = 1; nu = 0.3;
lambda_plane_stress = E0*nu/(1-nu^2); 
mu_plane_stress = E0/(2*(1+nu));    
lambda = [lambda_plane_stress, 0];
mu = [mu_plane_stress, 0];
case(2) 
lambda=[1 0];
mu=[1 0.1];
end
...
[F(activedofs(3:2:end),1) F(activedofs(4:2:end),2)];
end
...
case(2)
Loop=2;
Xi0_e(3:2:end,1:2) = keMu(3:2:end,3:2:end)\[feMu(3:2:end,1) feMu(4:2:end,2)];
end
